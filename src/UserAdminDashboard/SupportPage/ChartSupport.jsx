...
const ChartSupport = ({id}) => {
    ...
    useEffect(() => {
        setLoading(true)
        axios.get(`https://shark-app-65c5t.ondigitalocean.app/common/api/v1/support/chat-support/${id}?page=${currentPage}&limit=${itemPerPage}&status=${supportStatus}`)
        .then(res => {
            if(res.status === 200){
                setLoading(false)
        ...
        <div>
            ...
            }
            {
                supportData && supportData.map(data => 
                    <div style={{cursor: 'pointer'}} onClick={() => navigate(`/support/${data._id}`)} className='p-2 my-1 rounded-md border md:flex justify-between items-center' key={data._id}>
                        <div>
