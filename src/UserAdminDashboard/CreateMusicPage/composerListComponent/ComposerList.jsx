...
const ComposerList = ({handleCancel, reFetchComposer}) => {

    const { userNameIdRoll, composer, setComposer } = useContext(AuthContext);
    ...
    },[userNameIdRoll, reFetchComposer])


    const handleData = (data) => {
      if(composer){
        const dubleCheck = composer.some(a => a._id === data._id);
        if(dubleCheck){
          toast.error("All ready added this Author")
        }else{
          setComposer([...composer, data])
        }
      }else{
        setComposer([data])
      }
            ...
            }
            {
              ...
              <div>
                  ...
                  <div className="flex justify-center items-center my-2">
                    <a className="btn btn-sm btn-neutral rounded-full" href="https://app.dreamrecords.in/create-artist" target={'_blank'}>Add Author</a>
                  </div>
              </div>
            }
        </div>
